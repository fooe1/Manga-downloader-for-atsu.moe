...
for i in $(seq 1 20); do
    sleep 0.5
    if curl -s http://localhost:7337 > /dev/null 2>&1; then
        break
    fi
done
...
if command -v xdg-open &>/dev/null; then
    ...
    open "http://localhost:7337"   # macOS fallback
fi

echo "  Browser opened at http://localhost:7337"
...
# ── Step 6: Create a .desktop file for double-clicking on Linux ───────────────
DESKTOP_FILE="Manga Downloader.desktop"
INSTALL_DIR="$(pwd)"
...
Comment=Download manga from atsu.moe
Exec=bash -c "cd \'$INSTALL_DIR\' && source .venv/bin/activate && python app.py & sleep 3 && xdg-open http://localhost:7337"
...
# Place on Desktop if it exists
if [ -d "$HOME/Desktop" ]; then
    cp "$DESKTOP_FILE" "$HOME/Desktop/$DESKTOP_FILE"
    chmod +x "$HOME/Desktop/$DESKTOP_FILE"
    success "Shortcut placed on Desktop."
else
    success ".desktop shortcut created in current folder."
fi
